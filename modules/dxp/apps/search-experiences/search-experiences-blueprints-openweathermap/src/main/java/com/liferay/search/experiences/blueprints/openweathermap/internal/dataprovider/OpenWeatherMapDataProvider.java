...
public class OpenWeatherMapDataProvider {
	...
	public JSONObject getWeatherDataJSONObject(
		...
		if (!_validateConfiguration(problemsHolderBuilder)) {
			return null;
		}
		...
		JSONObject weatherDataJsonObject2 = _fetchOpenWeatherMapDataJSONObject(
			geoLocationPoint, problemsHolderBuilder);

		if (!_validateJSONResponseData(
				weatherDataJsonObject2, problemsHolderBuilder)) {
	...
	private JSONObject _createOpenWeatherMapDataJSONObject(
		...
		if (Validator.isBlank(rawData)) {
			problemsHolderBuilder.addProblem(
				_problemBuilders.error(
					getClass().getName(),
					"openweathermap.error.response-was-empty", rawData, null,
					null, new Throwable("OpenWeatherMap response was empty")
				).build());
		...
		try {
			...
			problemsHolderBuilder.addProblem(
				_problemBuilders.error(
					getClass().getName(),
					"openweathermap.error.invalid-response-format", rawData,
					null, null, jsonException
				).build());
		}
	...
	private boolean _validateConfiguration(
		...
		if (Validator.isBlank(_openWeatherMapConfiguration.apiKey())) {
			problemsHolderBuilder.addProblem(
				_problemBuilders.error(
					getClass().getName(),
					"openweathermap.error.api-key-must-be-configured", null,
					null, null,
		...
		if (Validator.isBlank(_openWeatherMapConfiguration.apiURL())) {
			problemsHolderBuilder.addProblem(
				_problemBuilders.error(
					getClass().getName(),
					"openweathermap.error.api-url-must-be-configured", null,
					null, null,
