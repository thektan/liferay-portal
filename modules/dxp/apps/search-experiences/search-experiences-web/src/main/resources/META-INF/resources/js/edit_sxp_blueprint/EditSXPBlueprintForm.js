...
function EditSXPBlueprintForm({
		...
		try {
			...
			if (!showSubmitWarningModal) {
				const validateErrors = await fetch(
					'/o/search-experiences-rest/sxp-blueprints/validate',
					{
						body: formData,
						method: 'POST',
					}
			...
			const responseContent = await fetch(
				`/o/search-experiences-rest/sxp-blueprints/${sxpBlueprintId}`,
				{
					body: formData,
					method: 'PATCH',
				}
	...
	const _handleFetchPreviewSearch = (value, delta, page, attributes) => {
		...
		return fetch('/o/search-experiences-rest/v1.0/search', {
			body: formData,
			method: 'POST',
		})
			.then((response) => {
...
EditSXPBlueprintForm.propTypes = {
	...
	keywordQueryContributors: PropTypes.arrayOf(PropTypes.string),
	modelPrefilterContributors: PropTypes.arrayOf(PropTypes.string),
	queryPrefilterContributors: PropTypes.arrayOf(PropTypes.string),
