...
@Component(
	...
	property = {
		...
		"mvc.command.name=" + SXPBlueprintMVCCommandNames.DELETE_SXP_BLUEPRINT
	},
	service = MVCActionCommand.class
)
public class DeleteSXPBlueprintMVCActionCommand extends BaseMVCActionCommand {
	...
	private void _delete(ActionRequest actionRequest) {
		long[] deleteIds = _getSXPBlueprintIds(actionRequest);

		try {
			for (long sxpBlueprintId : deleteIds) {
				_sxpSXPBlueprintService.deleteSXPBlueprint(sxpBlueprintId);
			}
		}
	...
	private long[] _getSXPBlueprintIds(ActionRequest actionRequest) {
		long[] sxpBlueprintIds = null;

		long sxpBlueprintId = SXPBlueprintRequestUtil.getSXPBlueprintId(
			actionRequest);

		if (sxpBlueprintId > 0) {
			sxpBlueprintIds = new long[] {sxpBlueprintId};
		}
		else {
			sxpBlueprintIds = ParamUtil.getLongValues(
				actionRequest, SXPBlueprintWebKeys.ROW_IDS);
		}

		return sxpBlueprintIds;
	}

	private static final Log _log = LogFactoryUtil.getLog(
		DeleteSXPBlueprintMVCActionCommand.class);
